...
export default function Navbar({ site = DEFAULT_SITE }: { site?: SiteContent }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-bg-stone/90 backdrop-blur-md border-b border-black/10" : "bg-transparent"}`}>
      <nav className="flex items-center justify-between px-6 md:px-10 py-4">
        <a href="#hero" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="font-headline text-sm md:text-base tracking-widest uppercase text-text-primary hover:text-accent transition-colors duration-300 font-black">{site.brandName}</a>
        ...
        <div className="flex items-center gap-4">
          <a href={site.bookHref} onClick={(e) => { e.preventDefault(); handleNavClick(site.bookHref); }} className="hidden md:inline-flex items-center gap-1 text-xs tracking-widest uppercase text-text-primary border border-text-primary px-4 py-2 hover:bg-text-primary hover:text-text-light transition-colors duration-300">{site.bookLabel} <span className="ml-1">↗</span></a>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-500 ${menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"} bg-bg-stone/95 backdrop-blur-md border-t border-black/10`}>
        <div className="flex flex-col px-6 py-6 gap-6">
          ...
          <a href={site.bookHref} onClick={(e) => { e.preventDefault(); handleNavClick(site.bookHref); }} className="inline-flex items-center gap-1 text-sm tracking-widest uppercase text-text-primary border border-text-primary px-4 py-2 hover:bg-text-primary hover:text-text-light transition-colors duration-300">{site.bookLabel} ↗</a>
        </div>
      </div>
    </header>
  );
